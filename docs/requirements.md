...
- **DASH-2** Metrics use these definitions:
  ...
  - Total applications = all non-archived applications.
  - Active applications = non-archived applications whose current status is Saved, Preparing, Applied, or Interview.
  - Interviews = non-archived applications whose status history ever contains `to_status = interview` (unique applications, not interview events).
  - Offers = non-archived applications whose status history ever contains `to_status = offer` (unique applications).
  - Upcoming deadlines = non-archived Saved/Preparing applications with `today <= deadline <= today + 7` calendar days (America/Toronto product timezone).
  - Interview rate = interviews / applied denominator × 100; offer rate = offers / applied denominator × 100. When the denominator is zero the data layer returns `null`, the UI shows `—` with "No applied applications yet", and no NaN/Infinity/fake percentage is ever rendered. Percentages display at most one decimal place.
...
- **DASH-4** Action lists: upcoming deadlines, recently updated applications, and applications requiring action. Requiring action uses only persisted rules, each application appearing once with its highest-priority reason:
  - Priority 1 "Deadline passed": non-archived Saved/Preparing with `deadline < today`.
  - Priority 2 "Apply before deadline": non-archived Saved/Preparing with `today <= deadline <= today + 3` calendar days.
  - Sorting: priority ASC, deadline ASC, `updated_at` DESC, id ASC; capped at 5.
  - Upcoming deadlines and recently updated lists are also capped at 5 with deterministic tie-breakers.
...
**Resolved definitions (2026-08-02):** submitted-over-time includes only
applications in the applied denominator. Each application's analytics
submission date is its stored `date_applied` when present; otherwise the
earliest status event with `to_status IN (applied, interview, offer,
rejected, withdrawn)` converted to America/Toronto calendar date. `created_at`
is never used as a fallback, and the derived date is never written back to
`date_applied`. The chart aggregates by `YYYY-MM` over all persisted
qualifying months in ascending order with an explicit empty state. Most
requested skills counts distinct non-archived applications per normalized
skill name (required and preferred both included), returns total/required/
preferred application counts, caps at Top 10, and orders by
`total_count DESC, normalized_name ASC` with a deterministic display name.
...
| O-4  | The dashboard's "applications requiring action" list has no defined rule set.                                                                                                                                                       | **Resolved 2026-08-02:** persisted-only rules: Priority 1 "Deadline passed" (Saved/Preparing, deadline < today) and Priority 2 "Apply before deadline" (Saved/Preparing, today <= deadline <= today + 3); one entry per application with the highest priority; order priority/deadline/updated_at/id; max 5. No email, calendar, inactivity-threshold, or AI rules. |
...
| O-8  | The Dashboard "interviews" and "offers" summary cards are not defined as count of applications ever reaching the stage versus count of interview events.                                                                            | **Resolved 2026-08-02:** unique non-archived applications that ever reached the stage via status history (`to_status = interview` / `to_status = offer`), regardless of a later Rejected/Withdrawn current status; not interview-event counts.                                                                                                                      |
...
| O-12 | The "applications submitted over time" chart semantics for applications with a null `date_applied` are not defined.                                                                                                                 | **Resolved 2026-08-02:** only applied-denominator applications are charted; use stored `date_applied` when non-null, otherwise the earliest applied-stage status event converted to America/Toronto calendar date; never use `created_at`; never write the derived date back. Aggregate by `YYYY-MM`, ascending, real months only.                                  |
