...
- **PRO-3** Required onboarding fields block completion until valid. The exact required set is to be confirmed (Appendix A, O-13).
...
- **TRK-3** `date_applied` may remain null when a record is moved to Applied manually; the UI prompts for it when appropriate but never invents a date.
...
- **TRK-5** Cards show company, job title, location, deadline, latest match score (when one exists; see Appendix A, O-7), date applied, and textual status. Column counts match rendered cards; empty columns have usable empty states; cards link to the Job Detail page of the owning user only.
...
- **TRK-15** Upcoming deadlines are surfaced as reminders; the reminder window is to be confirmed (Appendix A, O-6).
- **TRK-16** Expired Saved/Preparing applications show an unapplied warning; applied-stage records do not receive the unapplied warning. Deadline states are derived from the stored calendar date and covered by boundary-date tests.
...
| O-6  | The "upcoming deadline" reminder window (for example, number of days) is not defined.                                                                                                                                               | Missing                                       |
| O-7  | Kanban card behavior for "latest match score" when no match analysis exists (placeholder vs. omit) is not specified.                                                                                                                | Missing                                       |
...
| O-11 | The exact prompt behavior and timing for `date_applied` when moving a record to Applied is not detailed in the implementation plan.                                                                                                 | Missing                                       |
...
| O-13 | The schema marks only `preferred_name` as required after onboarding, while the plan refers to "required onboarding fields" in the plural.                                                                                           | Conflicting                                   |
